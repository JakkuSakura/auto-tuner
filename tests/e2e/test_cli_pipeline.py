...
def test_cli_run_creates_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    result = runner.invoke(app, ["run", "--config", "tests/fixtures/sample_experiment.toml"])
    ...
    assert (run_dir / "gpu_stats.jsonl").exists()
...
def test_cli_list_runs_shows_created_run(tmp_path: Path, monkeypatch) -> None:
    ...
    run_result = runner.invoke(app, ["run", "--config", "tests/fixtures/sample_experiment.toml"])
    assert run_result.exit_code == 0

    list_result = runner.invoke(
        app, ["list-runs", "--config", "tests/fixtures/sample_experiment.toml"]
    )
...
def test_cli_export_and_delete_run(tmp_path: Path, monkeypatch) -> None:
    ...
    run_result = runner.invoke(app, ["run", "--config", "tests/fixtures/sample_experiment.toml"])
    artifacts_line = next(
        line for line in run_result.stdout.splitlines() if line.startswith("artifacts=")
    )
    run_dir = Path(artifacts_line.split("=", 1)[1])
    ...
    archive_line = next(
        line for line in export_result.stdout.splitlines() if line.startswith("archive=")
    )
